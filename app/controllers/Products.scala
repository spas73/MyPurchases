...
/**
  ...
  */
class Products @Inject()(c: ControllerComponents, messagesApi: MessagesApi, configuration: Configuration) extends AbstractController(c) with I18nSupport {
  ...
  /**
    ...
    * @return the new form
    */
  private def makeProductForm(error: String, constraint: (Long) => Boolean) =
    Form(
      mapping(
        "ean" -> longNumber
  ...
  /**
    ...
    */
  private val productForm =
    makeProductForm("validation.ean.duplicate", isUniqueEan(_))
  ...
  def save = Action { implicit request =>
    ...
    newProductForm.fold(
      ...
      success = { newProduct =>
        ...
        val successMessage = ("success" -> Messages("products.new.success",
                                                    newProduct.name))
    ...
    if (Product.findByEan(ean).isEmpty)
      ...
      updatedProductForm.fold(
        ...
        success = { updatedProduct =>
          ...
          val successMessage = "success" -> Messages("products.update.success",
                                                     updatedProduct.name)
