...
import cuzfrog.utils.{DateParseTool, EncryptTool}
...
private[maila] object Configuration {
  ...
  def withKey(aesKey: Array[Byte]): Configuration = {
    ...
    new TypesafeConfiguration with EncryptedPw {
      override def key: Array[Byte] = aesKey: Array[Byte]
    }
  }
  ...
  private abstract class TypesafeConfiguration extends Configuration {
    ...
    override val serverProps: Properties = propsFromConfig(config.getConfig("server"))
    ...
    override val javaMailDebug: Boolean = config.getBoolean("server.javax.mail.debug")
    override val folderName: String = config.getString("reader.folder")
    override val isDeleteAfterFetch: Boolean = config.getBoolean("reader.delete-after-fetch")
    ...
    override val locale: Locale = Locale.forLanguageTag(config.getString("reader.pop3-received-date-parse.locale"))
    ...
    implicit private class RichConfig(in: Config) {
      def getEncryptedString(path: String, aesKey: Array[Byte]): String = {
        val decrypted = try {
          ...
          case (_: IllegalBlockSizeException | _: BadPaddingException) =>
